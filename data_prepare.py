...
from graphcare_.task_fn import drug_recommendation_fn, mortality_prediction_mimic3_fn, readmission_prediction_mimic3_fn, length_of_stay_prediction_mimic3_fn
import pickle
import json
...
def load_dataset(load_processed_dataset, dataset, task):
    ...
    if load_processed_dataset:
        ...
        with open(file_name, 'rb', encoding='utf-8', errors='ignore') as f:
        ...
        condition_dict = {}
        with open(condition_mapping_file, newline='', encoding='utf-8', errors='ignore') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                condition_dict[row['code']] = row['name'].lower()

        procedure_dict = {}
        with open(procedure_mapping_file, newline='', encoding='utf-8', errors='ignore') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                procedure_dict[row['code']] = row['name'].lower()

        drug_dict = {}
        with open(drug_file, newline='', encoding='utf-8', errors='ignore') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                if row['level'] == '3.0':
...
def load_embeddings(task):
    if task == "drugrec" or task == "lenofstay":
        with open('./graphs/cond_proc/CCSCM_CCSPROC/ent2id.json', 'r', encoding='utf-8', errors='ignore') as file:
            ent2id = json.load(file)
        with open('./graphs/cond_proc/CCSCM_CCSPROC/rel2id.json', 'r', encoding='utf-8', errors='ignore') as file:
            rel2id = json.load(file)
        with open('./graphs/cond_proc/CCSCM_CCSPROC/entity_embedding.pkl', 'rb', encoding='utf-8', errors='ignore') as file:
            ent_emb = pickle.load(file)
        with open('./graphs/cond_proc/CCSCM_CCSPROC/relation_embedding.pkl', 'rb', encoding='utf-8', errors='ignore') as file:
            rel_emb = pickle.load(file)

    elif task == "mortality" or task == "readmission":
        with open('./graphs/cond_proc_drug/CCSCM_CCSPROC_ATC3/ent2id.json', 'r', encoding='utf-8', errors='ignore') as file:
            ent2id = json.load(file)
        with open('./graphs/cond_proc_drug/CCSCM_CCSPROC_ATC3/rel2id.json', 'r', encoding='utf-8', errors='ignore') as file:
            rel2id = json.load(file)
        with open('./graphs/cond_proc_drug/CCSCM_CCSPROC_ATC3/entity_embedding.pkl', 'rb', encoding='utf-8', errors='ignore') as file:
            ent_emb = pickle.load(file)
        with open('./graphs/cond_proc_drug/CCSCM_CCSPROC_ATC3/relation_embedding.pkl', 'rb', encoding='utf-8', errors='ignore') as file:
            rel_emb = pickle.load(file)
...
def process_graph(dataset, task, sample_dataset, ent2id, rel2id, map_cluster, map_cluster_inv, map_cluster_rel, map_cluster_inv_rel, save_graph=False):
    ...
    for patient in tqdm(sample_dataset):
        ...
        for condition in conditions:
            ...
            with open(cond_file, 'r', encoding='utf-8', errors='ignore') as f:
                lines = f.readlines()
            for line in lines:
    ...
    if save_graph:
        with open(f'{path}/graph_{dataset}_{task}_th015.pkl', 'wb', encoding='utf-8', errors='ignore') as f:
            pickle.dump(G, f)
...
def process_sample_dataset(dataset, task, sample_dataset, G_tg, ent2id, rel2id, map_cluster, map_cluster_inv, map_cluster_rel, map_cluster_inv_rel, save_dataset=False):
    ...
    if save_dataset:
        with open(f'{path}/sample_dataset_{dataset}_{task}_th015.pkl', 'wb', encoding='utf-8', errors='ignore') as f:
